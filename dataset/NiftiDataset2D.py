...
class NiftiDataset:
    ...
    def __init__(self, data_dir='', image_filenames='', label_filename='', transforms3D=None, transforms2D=None,
                 train=False, labels=[0, 1], min_pixel=5, drop_ratio=0.1):
    ...
    def get_dataset(self):
        ...
        dataset = dataset.map(
            lambda case, slice_num: tuple(tf.py_func(self.input_parser, [case, slice_num], [tf.float32, tf.int32])),
    ...
    def input_parser(self, case, slice_num):
        ...
        if self.train:
            ...
            if not (sameSize and sameSpacing and sameDirection):
                raise Exception(
                    'Header info inconsistent: {}'.format(os.path.join(self.data_dir, case, self.label_filename)))
                exit()
